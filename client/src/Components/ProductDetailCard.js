...
export default function ProductDetailCard({ product, vendorName, user }) {
  ...
  async function reportProduct() {
    try {
      ...
      await axios
        .post("https://building-it-system-server.vercel.app/report-product", reportData, {
  ...
  const addProduct = async (productId) => {
    ...
    try {
      const res = await axios.get(
        `https://building-it-system-server.vercel.app/add-product/${productId}`,
        { params: { quantity: quantity }, withCredentials: true }
      );
      setMsg(res.data.msg);
      setError("");
      setQuantity(1);
    ...
    if (openModal) {
      setTimeout(() => {
        ...
        const newClasses = classes.filter(
          (c) =>
            !c.startsWith("dark:bg-gray") && !c.startsWith("dark:hover:bg-gray")
        );
          ...
          <div className="">
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <Rating theme={customTheme} size="md">
                  {[...Array(5)].map((_, i) => (
                    <Rating.Star
                      ref={myRef}
                      key={i}
                      filled={i < Math.floor(product.ratings + 0.5)}
                      className="!w-6 !h-6"
                    />
                  ))}
                </Rating>
              ...
              >
                <div id="myUniqueModalId">
                  <Modal.Header>
                    <div>
                      <p className="text-sm font-medium text-[#E61E2A]">
                        Product Name:
                        <span className="font-light text-gray-500 text-sm line-clamp-1">
                          Havells Velocity Neo High Speed 400mm Table Fan
                          (White)
                        </span>
                      </p>
                    </div>
                  ...
                  <Modal.Body className="overflow-y-auto">
                    ...
                    >
                      ...
                      <option value="Fake/Replica">
                        Product is fake/replica
                      </option>
                      <option value="Unidentified Product Origin">
                        The product's origin is unidentified
                      </option>
                      <option value="Indistinct Product Illustratior">
                        The product's illustrator is indistinct
                      </option>
                      <option value="Unmatched Product Name">
                        The product's name does not match with a product
                        illustrator
                      </option>
                  ...
                  <Modal.Footer>
                    ...
                    <button
                      ...
                      type="button"
                      class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-full text-sm px-5 py-2.5 me-2 mb-2 dark:border-gray-600"
                    >
                      Cancel
                    </button>
                  </Modal.Footer>
            ...
            <div className="flex">
              <button
                ...
                className="w-48 h-12 xs:text-[15px] text-lg text-white font-medium bg-[#EAB308] border-0  focus:outline-none hover:bg-[#EAA000] rounded-lg"
              >
                Add to Cart
              </button>
              <button
                onClick={() => buyProduct(product)}
                className="w-48 h-12 xs:text-[15px] text-lg ml-10 text-white font-medium bg-red-500 border-0 focus:outline-none hover:bg-red-500 rounded-lg"
              >
                Buy Now
              </button>
            </div>
