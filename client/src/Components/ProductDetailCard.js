...
export default function ProductDetailCard({ product, vendorName, user }) {
    ...
    async function reportProduct() {
        try {
            ...
            await axios.post('https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/report-product', reportData, { withCredentials: true })
                .then(res => {
                    setError('')
                    setMsg(res.data)
                    setTitle('')
    ...
    const addProduct = async (productId) => {
        notify()
        try {
            const res = await axios.get(`https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/add-product/${productId}`, { params: { quantity: quantity }, withCredentials: true });
            setMsg(res.data.msg);
            setError('');
            setQuantity(1);
        } catch (er) {
