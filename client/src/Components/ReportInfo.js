...
export default function ReportInfo({reports}) {
  ...
  const banUser = useCallback(async () => {
    try {
      const res = await axios.post("https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/ban-user", { userId: params.id, startDate: startDate, endDate: endDate }, { withCredentials: true });
  ...
  const removeUser = useCallback(async () => {
    try {
      const res = await axios.post("https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/ban-user", { remove: true, userId: params.id }, { withCredentials: true });
      toast.success(res.data.msg, {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: true,
