...
export default function CustomerReview({ product, setComments, comments }) {
  ...
  const replyComment = async (commentId) => {
    try {
      const res = await axios.post(`https://building-it-system-frontend.vercel.app/product/${commentId}/reply-comment`, { replyText: replyText }, { withCredentials: true });
  ...
  const fetchComments = useCallback(async () => {
    try {
      const res = await axios.get(`https://building-it-system-frontend.vercel.app/product/${product._id}/view-comment`, { withCredentials: true });
  ...
  const likeComment = async (commentId) => {
    try {
      await axios.post(`https://building-it-system-frontend.vercel.app/product/${commentId}/like`, {}, { withCredentials: true });
      setTimeout(() => {
        window.location.reload();
      }, 1000);
    } catch (err) {
