...
export default function AdminManageVendorProduct() {
  ...
  const getProducts = useCallback(async () => {
    const res = await axios.get("https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/admin/manage-product/query=", { withCredentials: true });
  ...
  const serachProduct = useCallback(async () => {
    const res = await axios.get(`https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/admin/manage-product/query=${searchTerm}`, { withCredentials: true });
    setProducts(res.data.products);
  }, [searchTerm])
