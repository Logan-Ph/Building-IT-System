...
export default function ManageUserPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await axios.get("https://building-it-system-server.vercel.app/admin/manage-user", {
  ...
  const customTheme = {
    ...
    tablist: {
      ...
      styles: {
        ...
        underline:
          "flex-wrap -mb-px border-b border-gray-200 dark:border-gray-200",
        pills:
          "flex-wrap font-medium text-sm text-gray-500 dark:text-gray-500 space-x-2",
        fullWidth:
          "w-full text-sm font-medium divide-x divide-gray-200 shadow grid grid-flow-col dark:divide-gray-200  rounded-none",
      },
      tabitem: {
    ...
    root: {
      ...
      shadow:
        "absolute bg-white dark:bg-white w-full h-full top-0 left-0 rounded-lg drop-shadow-md -z-10",
      wrapper: "relative",
    },
    body: {
      base: "group/body",
    ...
    row: {
      ...
      striped:
        "odd:bg-white even:bg-gray-50 odd:dark:bg-white even:dark:bg-gray-50",
    },
  };
  ...
  return users
    .filter(
      (user) =>
        ...
        regex.test(user.distributionHub)
    )
    .map((user) => {
      return user;
    });
...
function UserTable({ data, dataImage, type, customTheme }) {
  ...
  return (
    <>
      <div className="p-4 bg-gray-100">
        <div className="relative">
          ...
          <LightModeTable hoverable>
            <Table.Head id="TableHeader">{getHeadCellContent()}</Table.Head>
                  ...
                  <Table.Row className="dark:hover:bg-gray-200 bg-white dark:border-gray-700 border-b border-gray-200">
                    <Table.Cell className="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap">
                      <img
                        src={
                          dataSlice[i].img
                            ? `data:image/jpeg;base64,${dataSlice[i].img}`
                      ...
                      <div class="flex items-center whitespace-nowrap">
                        <div
                          class={`h-2.5 w-2.5 rounded-full me-2 ${
                            item.reportCount ? "bg-red-500" : "bg-green-500"
                          }`}
                        ></div>
      ...
      {!searchTerm && Math.ceil(data.length / 10) > 1 && (
        <Pagination
