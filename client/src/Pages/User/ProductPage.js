...
export default function TestingPage() {
  ...
  const createThread = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/chat", { vendorId: product.owner }, { withCredentials: true });
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await axios.get(`https://building-it-system-server-ppt2mxwor-logan-phs-projects.vercel.app/product/${params.id}`, { withCredentials: true });
