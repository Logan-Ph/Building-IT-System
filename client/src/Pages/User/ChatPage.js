...
export default function ChatPage() {
  return (
    <>
      ...
      const mainChatArea = document.querySelector(".mainChatArea");
      const contactListItems = document.querySelectorAll(
        ".contactList > .flex"
      );
      const backButton = document.querySelector(".backButton");

      // Function to toggle sidebar and chat area
      function toggleChatArea() {
        if (window.innerWidth < 768) {
          sidebar.classList.remove("w-full");
          sidebar.classList.add("hidden");
          mainChatArea.classList.remove("hidden");
          mainChatArea.classList.toggle("w-full");
        }
      ...
      contactListItems.forEach((item) => {
        item.addEventListener("click", () => {
          // Show chat area when a contact is clicked
          toggleChatArea();
        });
      });
      ...
      function goBackToSidebar() {
        if (window.innerWidth < 768) {
          ...
          mainChatArea.classList.toggle("hidden");
        }
      }
