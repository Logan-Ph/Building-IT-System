...
export default function ChatPage() {
  ...
  const getThreads = useCallback(async () => {
    try {
      const res = await axios.get("https://building-it-system-frontend.vercel.app/chat", { withCredentials: true });
  ...
  const sendMessage = async (e) => {
    ...
    try {
      if (!message) return;
      const res = await axios.post(`https://building-it-system-frontend.vercel.app/chat/${threads[threadIndex]._id}`, { content: message }, { withCredentials: true });
